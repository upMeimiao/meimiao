...
class DealWith {
  ...
  bolo(verifyData, callback) {
    const htmlUrl = verifyData.remote,
      userVal = verifyData.verifyCode.replace(/\s/g, ""),
      urlObj = URL.parse(htmlUrl, true),
      vid = urlObj.query.videoId,
      option = {
        url: `http://bolo.163.com/bolo/api/video/commentList.htm?videoId=${vid}&pageNum=1&pageSize=-1&type=0`
      },
      user = {};
    request.get(logger, option, (err, result) => {
      if (err) {
        logger.error('评论列表请求失败', err);
        this.bolo(verifyData, callback);
        return;
      }
      try {
        result = JSON.parse(result.body);
        ...
        callback(e, { code: 103, p: 44 });
        return;
      }
      if (!result.data || result.data.length <= 0) {
        callback('no-data', { code: 103, p: 44 });
        return;
      }
      const length = result.totalCommentCount;
      for (const comment of result.data) {
        if (userVal == comment.content.replace(/\s/, '')) {
          user.p = 44;
          user.id = comment.id;
          user.name = comment.nick;
          callback(null, user);
          return;
        }
      }
      callback(null, { code: 105, p: 44 });
    });
  }
}
