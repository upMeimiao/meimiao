 ...
 * @returns {*}
 */
const verify = (data, production) => {
  if (typeof data !== 'object' || Array.isArray(data)) {
    if (!production) throw new Error('数据不是JavaScript对象');
    return false;
  }
  if (!data.author || !data.platform || !data.bid || !data.aid || !data.title) {
    if (!production) throw new Error('数据关键字段缺失');
    return false;
  }
  for (const [key, value] of Object.entries(data)) {
    if (typeof value !== 'string' && typeof value !== 'number') {
      if (!production) throw new Error(`属性${key}数据不合法`);
      return false;
    }
    switch (key) {
      case 'play_num':
      ...
      case 'long_t':
        if (isNaN(value)) {
          if (!production) throw new Error(`属性${key}数据不是合法数字`);
          return false;
        }
 ...
 */
exports.saveCache = (db, listsName, data, logger) => {
  if (!data.cid) {
    const verifyRaw = verify(data, process.env.NODE_ENV);
    if (verifyRaw) {
      data = verifyRaw;
    } else {
      return;
...
exports.sendError = (db, email, callback) => {
  db.sadd('user:Facebook', email, (err, result) => {
    ...
    if (result === 1) {
      request({
        method: 'POST',
        url: 'http://spider-monitor.meimiaoip.com/api/alarm',
        form: {
