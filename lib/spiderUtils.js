...
exports.commentSnapshots = (db, raw) => {
  ...
  db.exists(key, (error, exists) => {
    ...
    if (exists === 1) {
      db.hget(key, 'newSnapshots', (err, newSnapshots) => {
        if (err) return;
        db.hmset(key, 'oldSnapshots', Number(newSnapshots), 'newSnapshots', Number(raw.comment_num));
      });
    }
  });
};
...
exports.sendError = (db, email, callback) => {
  db.sadd(`user:Facebook:${email}`, email, (err, result) => {
    if (err) {
      callback(err);
      return;
    }
