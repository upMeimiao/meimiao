        ...
        async.whilst(
            ...
            (cb) => {
                ...
                request.get( logger,option, ( err, result ) => {
                    ...
                    if(!result){
                        this.storaging.errStoraging('baomihua',option.url,task.id,"爆米花获取视频列表接口无返回数据","resultErr","list")
                        return cb()
                    }
                    if(!result.body){
                        this.storaging.errStoraging('baomihua',option.url,task.id,"爆米花获取视频列表接口无返回数据","resultErr","list")
        ...
        async.parallel([
            ...
            this.core.MSDB.hget(`apiMonitor:play_num`,"${media.author}_${media.aid}",(err,result)=>{
                ...
                if(result > media.play_num){
                    this.storaging.errStoraging('baomihua',`${api.baomihua.playNum}${task.id}&flvid=${media.aid}`,task.id,`爆米花视频${media.aid}播放量减少${result}(纪录)/${media.play_num}(本次)`,"playNumErr","playNum")
                    return
                }
            })
