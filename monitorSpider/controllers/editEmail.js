...
exports.interEmail = (events, message) => {
  ...
  switch (message.typeErr) {
    case 'error':
      emailContent += `<tr style="background: rgba(255, 0, 90, 0.55);"><td>${message.platfrom}</td><td>${message.bid}</td><td>${message.bname}</td><td>${message.typeErr}:${message.interface}</td><td>${message.url}</td><td>${message.message}</td><td>${message.lastTime}</td></tr></table>`;
      sendEmail.sendEmail(`${message.platform}平台 接口请求出错`, emailContent, 'red');
      break;
    case 'status':
      if (message.typeErr < 500 && message.typeErr >= 400) {
        emailContent += `<tr style="background: rgba(0, 74, 255, 0.42);"><td>${message.platfrom}</td><td>${message.bid}</td><td>${message.bname}</td><td>${message.typeErr}:${message.interface}</td><td>${message.url}</td><td>${message.message}</td><td>${message.lastTime}</td></tr></table>`;
        sendEmail.sendEmail(`${message.platform}平台 接口请求400状态`, emailContent, 'blue');
      } else {
        emailContent += `<tr style="background: rgba(211, 255, 0, 0.44);"><td>${message.platfrom}</td><td>${message.bid}</td><td>${message.bname}</td><td>${message.typeErr}:${message.interface}</td><td>${message.url}</td><td>${message.message}</td><td>${message.lastTime}</td></tr></table>`;
