...
class dealWith {
  ...
  getVideo(task, callback) {
    ...
    if (task.id.toString().length < 6) {
      ...
      request.get(logger, option, (err, result) => {
        ...
        if (!vidObj.length) {
          typeErr = {type: 'data', err: 'fengxing-video-dom-结构异常', interface: 'getVideo', url: option.url};
          infoCheck.interface(this.core, task, typeErr);
          return;
        }
        async.parallel(
  ...
  getVideoList(task, vidObj) {
    ...
    request.get(logger, option, (err, result) => {
      ...
      if (!dataJson || !dataJson.dvideos.length) {
        typeErr = {type: 'data', err: 'fengxing-视频号视频列表-数据异常', interface: 'getVideoList', url: option.url};
        infoCheck.interface(this.core, task, typeErr);
        return;
      }
  ...
  getVidList(task) {
    ...
    request.get(logger, option, (err, result) => {
      ...
      if (!result ||!result.episodes.length) {
        typeErr = {type: 'data', err: 'fengxing-原创-视频列表数据异常', interface: 'getVidList-非视频号', url: option.url};
        infoCheck.interface(this.core, task, typeErr);
        return;
      }
  ...
  getVideoInfo(task, vid) {
    ...
    if (task.type === '视频号') {
      ...
      request.get(logger, option, (err, result) => {
        ...
        if (!result) {
          typeErr = {type: 'data', err: 'fengxing-data-视频号-视频详情异常', interface: 'getVideoInfo-视频号', url: option.url};
          infoCheck.interface(this.core, task, typeErr);
        }
      ...
      request.get(logger, option, (err, result) => {
        ...
        if (!vidClass.length) {
          typeErr = {type: 'data', err: 'fengxing-原创-视频详情页DOM结构异常', interface: 'getVideoInfo-非视频号', url: option.url};
          infoCheck.interface(this.core, task, typeErr);
        }
        option = null; result = null;
      });
  ...
  getComment(task, vid) {
    ...
    request.get(logger, option, (err, result) => {
      ...
      if (!result.data) {
        typeErr = {type: 'data', err: 'fengxing-comment-数据异常', interface: 'getComment', url: option.url};
        infoCheck.interface(this.core, task, typeErr);
      }
      option = null; result = null;
    });
  ...
  getCreatTime(task, vid) {
    ...
    request.get(logger, option, (err, result) => {
      ...
      if (!result.data) {
        typeErr = {type: 'data', err: 'fengxing-发布时间-数据异常', interface: 'getCreatTime', url: option.url};
        infoCheck.interface(this.core, task, typeErr);
      }
      option = null; result = null;
    });
