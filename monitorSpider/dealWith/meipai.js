...
class dealWith {
  ...
  list(task, callback) {
    ...
    request.get(logger, option, (err, result) => {
      if (err) {
        if (err.status && err.status !== 200) {
          typeErr = {type: 'status', err: JSON.stringify(err.status), interface: 'list', url: options.url};
          infoCheck.interface(this.core, task, typeErr);
        } else {
          typeErr = {type: 'error', err: JSON.stringify(err.message), interface: 'list', url: options.url};
          infoCheck.interface(this.core, task, typeErr);
        }
        callback();
        return;
      ...
      try {
        ...
        typeErr = {type: 'json', err: JSON.stringify(e.message), interface: 'list', url: options.url};
        infoCheck.interface(this.core, task, typeErr);
        callback();
        return;
      }
      if (!result || result.length == 0) {
        typeErr = {type: 'data', err: 'meipai-list-data-null', interface: 'list', url: options.url};
        infoCheck.interface(this.core, task, typeErr);
      }
  ...
  deal(task, list) {
    ...
    request.get(logger, option, (err, result) => {
      if (err) {
        if (err.status && err.status !== 200) {
          typeErr = {type: 'status', err: JSON.stringify(err.status), interface: 'media', url: options.url};
          infoCheck.interface(this.core, task, typeErr);
        } else {
          typeErr = {type: 'error', err: JSON.stringify(err.message), interface: 'media', url: options.url};
          infoCheck.interface(this.core, task, typeErr);
        }
        return;
      }
      try {
        ...
        typeErr = {type: 'json', err: JSON.stringify(e.message), interface: 'media', url: options.url};
        infoCheck.interface(this.core, task, typeErr);
      }
    });
  }
