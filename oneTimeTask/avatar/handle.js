...
exports.iqiyi = (user) => {
    return new Promise((resolve, reject)=>{
        let url = `http://www.iqiyi.com/u/${user.bid}`
        request( url, ( error, response, body) => {
            ...
            avatar = avatar ? avatar : $('div.profile_pic img').attr('src')
            resolve({
                p: user.platform,
                id: user.bid,
                avatar: avatar
...
exports.tencent = (user) => {
    return new Promise((resolve, reject)=>{
        let url = `http://v.qq.com/vplus/${user.bid}`
        request( url, (error, response, body) => {
            if(error){
                return reject(error.message)
            }
...
exports.meipai = (user) => {
    return new Promise((resolve, reject)=>{
        let url = `https://newapi.meipai.com/users/show.json?id=${user.bid}`
        request(url, (error, response, body) => {
            if(error){
                return reject(error.message)
            }
...
exports.miaopai = (user) => {
    return new Promise((resolve, reject)=>{
        let url = `http://api.miaopai.com/m/shot_channel.json?version=6.3.7&os=ios&page=1&per=20&suid=${user.bid}`
        request(url, (error, response, body) => {
            if(error){
                return reject(error.message)
            }
...
exports.kuaibao = (user) => {
    return new Promise((resolve, reject)=>{
        let url = `http://r.cnews.qq.com/getSubItem?chlid=${user.bid}`
        request(url, (error, response, body) => {
            if(error){
                return reject(error.message)
            }
...
exports.yidian = (user) => {
    return new Promise((resolve, reject)=>{
        let url = `http://www.yidianzixun.com/api/q/?fields=docid&fields=category&fields=date&fields=image&fields=image_urls&fields=like&fields=source&fields=title&fields=url&fields=comment_count&fields=summary&fields=up&version=999999&infinite=true&path=channel|news-list-for-channel&channel_id=${user.bid}&cstart=0&cend=10`
        request(url, (error, response, body) => {
            if(error){
                return reject(error.message)
            }
...
exports.tudou = (user) => {
    return new Promise((resolve, reject)=>{
       setTimeout(() => {
           let url = `http://user.api.3g.tudou.com/v4/channel/info?guid=7066707c5bdc38af1621eaf94a6fe779&pid=c0637223f8b69b02&id=${user.bid}`
           request(url, (error, response, body) => {
               if(error){
                   return reject(error.message)
               }
...
exports.ku6 = (user) => {
    return new Promise((resolve, reject)=>{
        let url = `http://boke.ku6.com/${user.bid}`
        request(url, (error, response, body) => {
            if(error){
                return reject(error.message)
            }
...
exports.btime = (user) => {
    return new Promise((resolve, reject)=>{
        let url = `http://record.btime.com/getUserChannel?uid=${user.bid}`
        request(url, (error, response, body) => {
            if(error){
                return reject(error.message)
            }
...
exports.weishi = (user) => {
    return new Promise((resolve, reject)=>{
        let url = 'http://weishi.qq.com/u/' + user.bid
        request(url, (error, response, body) => {
            if(error){
                return reject(error.message)
            }
...
exports.xiaoying = (user) => {
    return new Promise((resolve, reject)=>{
        let url = `https://w.api.xiaoying.co/webapi2/rest/user/detail?appkey=30000000&auid=${user.bid}`
        request(url, (error, response, body) => {
            if(error){
                return reject(error.message)
            }
...
exports.budejie = (user) => {
    return new Promise((resolve, reject)=>{
        let url = `http://api.budejie.com/api/api_open.php?a=profile&c=user&userid=${user.bid}`
        request(url, (error, response, body) => {
            if(error){
                return reject(error.message)
            }
...
exports.neihan = (user) => {
    return new Promise((resolve, reject)=>{
        let url = `http://isub.snssdk.com/neihan/user/profile/v2/?user_id=${user.bid}`
        request(url, (error, response, body) => {
            if(error){
                return reject(error.message)
            }
...
exports.yy = (user) => {
    return new Promise((resolve, reject)=>{
        let url = `http://www.yy.com/u/${user.bid}`
        request(url, (error, response, body) => {
            if(error){
                return reject(error.message)
            }
...
exports.weibo = (user) => {
    return new Promise((resolve, reject)=>{
        setTimeout(()=>{
            let url = `http://m.weibo.cn/container/getIndex?sudaref=m.weibo.cn&retcode=6102&type=uid&value=${user.bid}`
            request(url, (error, response, body) => {
                if(error){
                    return reject(error.message)
                }
                ...
                let avatar = body.userInfo.profile_image_url ? body.userInfo.profile_image_url : ''
                resolve({
                    p: user.platform,
                    id: user.bid,
                    avatar: avatar
...
exports.ifeng = (user) => {
    return new Promise((resolve, reject)=>{
        ...
        request(options, (error, response, body) => {
            ...
            let avatar = body.infoList[0].weMedia.headPic ? body.infoList[0].weMedia.headPic : ''
            resolve({
                p: user.platform,
                id: user.bid,
                avatar: avatar
...
exports.wangyi = (user) => {
    return new Promise((resolve, reject)=>{
        let url = `http://c.m.163.com/nc/topicset/home/android/${user.bid}.html`
        request(url, (error, response, body) => {
            if(error){
                return reject(error.message)
            }
            ...
            let avatar = body.topicSet.img.substring(0,1) !== 'T' ? body.topicSet.img : body.topicSet.topic_icons
            resolve({
                p: user.platform,
                id: user.bid,
                avatar: avatar
...
exports.uctt = (user) => {
    return new Promise((resolve, reject)=>{
        let url = `http://napi.uc.cn/3/classes/article/categories/wemedia/lists/${user.bid}?_app_id=cbd10b7b69994dca92e04fe00c05b8c2&_fetch=1&_fetch_incrs=1&_size=5&_max_pos=&uc_param_str=frdnsnpfvecpntnwprdsssnikt`
        request(url, (error, response, body) => {
            if(error){
                return reject(error.message)
            }
            ...
            let avatar = body.data[0].avatar_url ? body.data[0].avatar_url : ''
            resolve({
                p: user.platform,
                id: user.bid,
                avatar: avatar
...
exports.mgtv = (user) => {
    return new Promise((resolve, reject)=>{
        let url = `http://www.mgtv.com/h/${user.bid}.html?fpa=se`
        request(url, (error, response, body) => {
            if(error){
                return reject(error.message)
            }
...
exports.baijia = (user) => {
    return new Promise((resolve, reject)=>{
        ...
        let url = `http://baijiahao.baidu.com/api/content/article/listall?sk=super&ak=super&_skip=0&status=in:publish,published&_preload_statistic=1&_timg_cover=50,172,1000&_cache=1&app_id=${user.bid}&_limit=200`
        let arr = []
        request(url, (error, response, body) => {
            if(error){
                return reject(error.message)
...
exports.qzone = (user) => {
    return new Promise((resolve, reject)=>{
        let avatar = 'https://qlogo4.store.qq.com/qzone/'+ user.bid +'/'+ user.bid +'/100?'
        resolve({
            p: user.platform,
            id: user.bid,
            avatar: avatar
...
exports.cctv = (user) => {
    return new Promise((resolve, reject)=>{
        let url = `http://my.xiyou.cntv.cn/api/user/getuserinfo?&format=json&visit_id=0&snap=30&user_id=${user.bid}`
        request(url, (error, response, body) => {
            if(error){
                return reject(error.message)
            }
            ...
            let isNo = body.data.user_head.toString().indexOf('30x30')
            if(isNo){
                resolve({
                    p: user.platform,
                    id: user.bid,
...
exports.xinlan = (user) => {
    return new Promise((resolve, reject)=>{
        let url = 'http://so.cztv.com/pc/s?wd='+ encodeURIComponent(user.bname)
        request(url, (error, response, body) => {
            if(error){
                return reject(error.message)
            }
...
exports.huashu = (user) => {
    return new Promise((resolve, reject)=>{
        let url = 'http://www.wasu.cn/Agginfo/index/id/'+user.bid
        request(url, (error, response, body) => {
            if(error){
                return reject(error.message)
            }
...
exports.baiduVideo = (user) => {
    return new Promise((resolve, reject)=>{
        let url = 'http://v.baidu.com/tagapi?type=2&tag='+ encodeURIComponent(user.bname) +'&_='+ new Date().getTime()
        request(url, (error, response, body) => {
            if(error){
                return reject(error.message)
            }
            ...
            let avatar = body.data[0].tag_info ? body.data[0].tag_info.bigimgurl : ''
            resolve({
                p: user.platform,
                id: user.bid,
                avatar: avatar
