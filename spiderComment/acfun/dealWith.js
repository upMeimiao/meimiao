...
class dealWith {
	...
	totalPage( task, callback ){
		...
		request.get( logger, option, (err, result) => {
			...
			let comment = result.data.commentContentArr['c'+result.data.commentList[0]],
				time = new Date(comment.postDate)
	...
	deal( task, comments, callback ){
		...
		async.whilst(
			...
			(cb) => {
				let commentData = comments.commentContentArr['c'+comments.commentList[index]],
