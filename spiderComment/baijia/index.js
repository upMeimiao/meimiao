...
class spiderCore {
  ...
  deal() {
    ...
    queue.process('comment_baijia', this.settings.concurrency, (job, done) => {
      logger.trace('Get baijia task!');
      const work = job.data,
    ...
    queue.process('comment_update_baijia', this.settings.concurrency, (job, done) => {
      logger.trace('Get baijia task!');
      const work = job.data,
        key = `c:${work.p}:${work.aid}`;
      logger.info(work);
