...
class spiderCore {
  ...
  test() {
    const work = {
      bid: '45123',
      aid: '30l8bq9am8s921o5lsolbd5c5ha',
      p: 15,
      taskType: 0,
      commentId: 0,
      commentTime: 0,
  ...
  deal() {
    ...
    queue.process('comment_btime', this.settings.concurrency, (job, done) => {
      logger.trace('Get btime task!');
      const work = job.data,
    ...
    queue.process('comment_update_btime', this.settings.concurrency, (job, done) => {
      logger.trace('Get btime task!');
      const work = job.data,
        key = `c:${work.p}:${work.aid}`;
      logger.info(work);
