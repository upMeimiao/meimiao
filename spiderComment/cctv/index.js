...
class spiderCore {
  ...
  test() {
    const work = {
      bid: '18907911',
      aid: 'f64ef0ca-6736-11e3-8b06-e60566ef7fac',
      p: 30,
      taskType: 0,
      commentId: 0,
      commentTime: 0,
  ...
  deal() {
    ...
    queue.process('comment_cctv', this.settings.concurrency, (job, done) => {
      logger.trace('Get cctv task!');
      const work = job.data,
    ...
    queue.process('comment_update_cctv', this.settings.concurrency, (job, done) => {
      logger.trace('Get cctv task!');
      const work = job.data,
        key = `c:${work.p}:${work.aid}`;
      logger.info(work);
