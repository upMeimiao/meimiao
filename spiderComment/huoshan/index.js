...
class spiderCore {
  ...
  deal() {
    ...
    queue.process('comment_huoshan', this.settings.concurrency, (job, done) => {
      logger.trace('Get huoshan task!');
      const work = job.data,
    ...
    queue.process('comment_update_huoshan', this.settings.concurrency, (job, done) => {
      logger.trace('Get huoshan task!');
      const work = job.data,
        key = `c:${work.p}:${work.aid}`;
      logger.info(work);
