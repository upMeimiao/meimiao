...
class spiderCore {
  ...
  deal() {
    ...
    queue.process('comment_ku6', this.settings.concurrency, (job, done) => {
    ...
    queue.process('comment_update_ku6', this.settings.concurrency, (job, done) => {
      logger.trace('Get ku6 task!');
      const work = job.data,
        key = `c:${work.p}:${work.aid}`;
      logger.info(work);
