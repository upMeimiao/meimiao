...
class spiderCore {
  ...
  deal() {
    ...
    queue.process('comment_kuaibao', this.settings.concurrency, (job, done) => {
      logger.trace('Get kuaibao task!');
      const work = job.data,
    ...
    queue.process('comment_update_kuaibao', this.settings.concurrency, (job, done) => {
      logger.trace('Get kuaibao task!');
      const work = job.data,
        key = `c:${work.p}:${work.aid}`;
      logger.info(work);
