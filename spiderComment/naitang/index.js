...
class spiderCore {
  ...
  deal() {
    ...
    queue.process('comment_naitang', this.settings.concurrency, (job, done) => {
      logger.trace('Get naitang task!');
      const work = job.data,
    ...
    queue.process('comment_update_naitang', this.settings.concurrency, (job, done) => {
      logger.trace('Get naitang task!');
      const work = job.data,
        key = `c:${work.p}:${work.aid}`;
      logger.info(work);
