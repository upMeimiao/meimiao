...
class spiderCore {
  ...
  deal() {
    ...
    queue.process('comment_qzone', this.settings.concurrency, (job, done) => {
    ...
    queue.process('comment_update_qzone', this.settings.concurrency, (job, done) => {
      logger.trace('Get qzone task!');
      const work = job.data,
        key = `c:${work.p}:${work.aid}`;
      logger.info(work);
