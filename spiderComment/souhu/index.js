...
class spiderCore {
  ...
  deal() {
    ...
    queue.process('comment_souhu', this.settings.concurrency, (job, done) => {
      logger.trace('Get souhu task!');
      const work = job.data,
    ...
    queue.process('comment_update_souhu', this.settings.concurrency, (job, done) => {
      logger.trace('Get souhu task!');
      const work = job.data,
        key = `c:${work.p}:${work.aid}`;
      logger.info(work);
