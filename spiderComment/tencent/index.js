...
class spiderCore {
  ...
  test() {
    const work = {
      bid: '6e96cc6f6a80eb8f51239ff4d928400f',
      aid: 'w0161tttpbg',
      p: 4,
      taskType: 0,
      commentId: 0,
  ...
  deal() {
    ...
    queue.process('comment_tencent', this.settings.concurrency, (job, done) => {
    ...
    queue.process('comment_update_tencent', this.settings.concurrency, (job, done) => {
      logger.trace('Get tencent task!');
      const work = job.data,
        key = `c:${work.p}:${work.aid}`;
      logger.info(work);
