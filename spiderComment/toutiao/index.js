...
class spiderCore {
  ...
  test() {
    const work = {
      bid: 4034695314,
      aid: '6382266108686107137', // 6266938511926231297
      p: 6,
      taskType: 0,
      commentId: 0,
      commentTime: 0,
  ...
  deal() {
    ...
    queue.process('comment_toutiao', this.settings.concurrency, (job, done) => {
    ...
    queue.process('comment_update_toutiao', this.settings.concurrency, (job, done) => {
      logger.trace('Get toutiao task!');
      const work = job.data,
        key = `c:${work.p}:${work.aid}`;
      logger.info(work);
