...
class spiderCore {
  ...
  test() {
    const work = {
      bid: '6c6087eca3d74840801043fd486e65b3',
      aid: '11660708452469751350',
      p: 26,
      taskType: 1,
      commentId: 0,
      commentTime: 0,
  ...
  deal() {
    ...
    queue.process('comment_uctt', this.settings.concurrency, (job, done) => {
      logger.trace('Get uctt task!');
      let work = job.data,
    ...
    queue.process('comment_update_uctt', this.settings.concurrency, (job, done) => {
      logger.trace('Get uctt task!');
      let work = job.data,
        key = `c:${work.p}:${work.aid}`;
      logger.info(work);
