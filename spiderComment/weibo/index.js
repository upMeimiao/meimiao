...
class spiderCore {
  ...
  deal() {
    ...
    queue.process('comment_weibo', this.settings.concurrency, (job, done) => {
    ...
    queue.process('comment_update_weibo', this.settings.concurrency, (job, done) => {
      logger.trace('Get weibo task!');
      const work = job.data,
        key = `c:${work.p}:${work.aid}`;
      logger.info(work);
