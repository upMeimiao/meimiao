...
class spiderCore {
  ...
  deal() {
    ...
    queue.process('comment_weishi', this.settings.concurrency, (job, done) => {
      logger.trace('Get weishi task!');
      const work = job.data,
    ...
    queue.process('comment_update_weishi', this.settings.concurrency, (job, done) => {
      logger.trace('Get weishi task!');
      const work = job.data,
        key = `c:${work.p}:${work.aid}`;
      logger.info(work);
