...
class spiderCore {
  ...
  deal() {
    ...
    queue.process('comment_youliao', this.settings.concurrency, (job, done) => {
      logger.trace('Get youliao task!');
      const work = job.data,
    ...
    queue.process('comment_update_youliao', this.settings.concurrency, (job, done) => {
      logger.trace('Get youliao task!');
      const work = job.data,
        key = `c:${work.p}:${work.aid}`;
      logger.info(work);
